<template>
  <div id="screen" class="flex flex-col items-center h-full" :class="!textLines ? 'justify-start' : 'justify-center'">
    <div id="text" class="mx-2 my-1">
      <div v-for="(line, index) in textLines" :key="index" class="flex self-end w-full text-2xl leading-5 truncate line-break">
        {{ line }}
      </div>
    </div>
    <div class="flex justify-center w-full text-xl antialiased leading-4">
      <Time></Time>
    </div>
  </div>
</template>
